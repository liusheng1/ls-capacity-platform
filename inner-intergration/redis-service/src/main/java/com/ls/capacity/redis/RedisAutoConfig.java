...
import com.ls.capacity.redis.serializer.RedisObjectSerializer;
...
public class RedisAutoConfig {
  ...
  public LettuceConnectionFactory lettuceConnectionFactory(
    ...
    LettuceClientConfiguration configuration = null;
    if (redisProperties.getSentinel() != null) {
      // 哨兵配置
      Method nodesMothed = ReflectionUtils.findMethod(Sentinel.class, "getNodes");
      Object nodesValue = ReflectionUtils.invokeMethod(nodesMothed, redisProperties.getSentinel());
      String[] nodes = null;
      ...
      redisConfiguration =
          new RedisSentinelConfiguration(
              redisProperties.getSentinel().getMaster(), sentinelHostAndPorts);
      ...
      configuration =
        ...
        && org.springframework.util.ReflectionUtils.invokeMethod(clusterMethod, redisProperties)
            != null) {
      // 集群配置
      ...
      clusterNodes.forEach(
          address ->
              nodes.add(
                  new RedisNode(
                      address.split(":")[0].trim(), Integer.valueOf(address.split(":")[1]))));
    ...
    LettuceConnectionFactory lettuceConnectionFactory =
        new LettuceConnectionFactory(redisConfiguration, configuration);
  ...
  public RedissonClient redissonClient() throws IOException {
    Config config = null;
    Method clusterMethod = org.springframework.util.ReflectionUtils.findMethod(RedisProperties.class, "getCluster");
    Method timeoutMethod = org.springframework.util.ReflectionUtils.findMethod(RedisProperties.class, "getTimeout");
    Object timeoutValue = org.springframework.util.ReflectionUtils.invokeMethod(timeoutMethod, redisProperties);
    int timeout;
    if (null == timeoutValue) {
      timeout = 60000;
    } else if (!(timeoutValue instanceof Integer)) {
      Method millisMethod = org.springframework.util.ReflectionUtils.findMethod(timeoutValue.getClass(), "toMillis");
      timeout = ((Long) org.springframework.util.ReflectionUtils.invokeMethod(millisMethod, timeoutValue)).intValue();
    } else {
    ...
    if (redissonProperties.getConfig() != null) {
      ...
      // 哨兵配置
      Method nodesMethod = org.springframework.util.ReflectionUtils.findMethod(Sentinel.class, "getNodes");
      Object nodesValue = org.springframework.util.ReflectionUtils.invokeMethod(nodesMethod, redisProperties.getSentinel());
    ...
    } else if (clusterMethod != null && org.springframework.util.ReflectionUtils.invokeMethod(clusterMethod, redisProperties) != null) {
      // 集群配置
      Object clusterObject = org.springframework.util.ReflectionUtils.invokeMethod(clusterMethod, redisProperties);
      Method nodesMethod = org.springframework.util.ReflectionUtils.findMethod(clusterObject.getClass(), "getNodes");
      List<String> nodesObject = (List) org.springframework.util.ReflectionUtils.invokeMethod(nodesMethod, clusterObject);
      ...
      config = new Config();
      String prefix = "redis://";
      Method method = org.springframework.util.ReflectionUtils.findMethod(RedisProperties.class, "isSsl");
      if (method != null && (Boolean) org.springframework.util.ReflectionUtils.invokeMethod(method, redisProperties)) {
        prefix = "rediss://";
      }
